        ...
        return(
            <div>
                <div class = "topnav" align = "right">
                    <Link style={{padding:"0 10px"}} to={'/addquestion'}>Add Question </Link>
                {this.props.isAuthenticated?<div></div>:
                    <Link style={{padding:"0 10px"}} to={'/register'}>Register</Link>}
                {this.props.isAuthenticated?<div></div>:
                <Link style={{padding:"0 10px"}} to={'/login'}>  Login  </Link> }
                </div>
                <h2> Questions List </h2>
                {
                    ...
                    <div class="card"  style={{border:"1px groove",width:"80%",position:'relative'}}>
                        <div class="container">
                        <h2><Link  to={'/questions/'+currentObj.id +'/'}>{currentObj.title}</Link></h2>
